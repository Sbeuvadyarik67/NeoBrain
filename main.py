...
# ========== ЭНДПОИНТ /models (ДОБАВЛЕН ПЕРВЫМ) ==========
@app.get("/models")
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .waves-decor {
            ...
            -webkit-mask-image: radial-gradient(ellipse at 50% 100%, black 40%, transparent 80%);
            pointer-events: none;
            z-index: 5;
            opacity: 0.7;
        ...
        .waves-decor.left {
            ...
            background: repeating-linear-gradient(0deg, 
                ...
                transparent 3px,
                transparent 12px);
            mask-image: radial-gradient(ellipse at 0% 50%, black 40%, transparent 80%);
            -webkit-mask-image: radial-gradient(ellipse at 0% 50%, black 40%, transparent 80%);
        }

        .waves-decor.right {
            left: auto;
            right: 0;
            mask-image: radial-gradient(ellipse at 100% 50%, black 40%, transparent 80%);
            -webkit-mask-image: radial-gradient(ellipse at 100% 50%, black 40%, transparent 80%);
        }

        /* Анимация — волны движутся */
        @keyframes waveMove {
...
<script>
    ...
    window.addEventListener('click', function(event) {
        ...
        // --- ЗАГРУЗКА МОДЕЛЕЙ ИЗ OLLAMA (СНАРУЖИ)---
    async function loadModels() {
        try {
            ...
            if (data.models && data.models.length > 0) {
                ...
                modelSelect.innerHTML = '<option value="qwen2.5-coder:7b">qwen2.5-coder:7b</option><option value="qwen2.5-coder:1.5b">qwen2.5-coder:1.5b</option>';
            }
        } catch (err) {
    ...
    // --- УПРАВЛЕНИЕ ДЕКОРАТИВНЫМИ ВОЛНАМИ ---
    ...
    function applyWavesState() {
        ...
        if (wavesContainer) wavesContainer.style.display = enabled ? 'block' : 'none';
